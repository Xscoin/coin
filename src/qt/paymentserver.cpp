...
const int XSCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString XSCOIN_IPC_PREFIX("xscoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(XSCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(XSCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(XSCOIN_IPC_CONNECT_TIMEOUT);
